...
**Ngày lập:** 2026-08-24 · **Trạng thái:** Chờ anh duyệt trước khi triển khai Phase 1
...
**Công việc cụ thể:**
- [ ] 1.1. Logo concept dạng SVG (minimal, organic, warm — không dùng ảnh stock)
- [ ] 1.2. Design tokens: bảng màu, typography scale, spacing scale, radius, shadow, breakpoints
- [ ] 1.3. Bộ component: Button (primary/secondary/ghost + hover/focus/disabled), Card, Input/Form field, SectionHeader, Header (nav), Footer, Breadcrumb, Timeline, Accordion, Lightbox, Sticky CTA, Pagination
- [ ] 1.4. CSS files: `assets/css/style.css` (tokens + base + components), `responsive.css`, `animations.css` (tôn trọng prefers-reduced-motion)
...
**Công việc cụ thể:**
- [ ] 2.1. Sitemap đầy đủ: index, ve-chung-toi, chuong-trinh, moi-truong, hoat-dong, dinh-duong, hoc-phi, tuyen-sinh, gallery, tin-tuc, lien-he, faq, dang-ky-tham-quan + news/bai-viet-*.html + privacy-policy, terms, 404
- [ ] 2.2. URL tiếng Việt chuẩn SEO (viết thường, không dấu, gạch ngang): `/chuong-trinh`, `/hoc-phi`, `/dang-ky-tham-quan`…
- [ ] 2.3. Navigation phản ánh hành trình phụ huynh (không đặt "Tuyển sinh" lên đầu menu)
- [ ] 2.4. Breadcrumbs + internal linking giữa các trang
- [ ] 2.5. `robots.txt`, `sitemap.xml`, favicon (SVG inline)
...
- [ ] 3.1. Hero: headline chạm cảm xúc + phụ đề + 2 CTA (ĐĂNG KÝ THAM QUAN / XEM HỌC PHÍ)
- [ ] 3.2. Trust bar: Yêu thương · An toàn · Phát triển · Dinh dưỡng
- [ ] 3.3. Giới thiệu ngắn + số liệu (để placeholder)
- [ ] 3.4. Chương trình giáo dục (4-6 thẻ dẫn sang chuong-trinh.html)
- [ ] 3.5. Một ngày của bé (timeline 6-8 mốc)
- [ ] 3.6. Môi trường (3-4 khu tiêu biểu → moi-truong.html)
- [ ] 3.7. An toàn (camera, khử khuẩn, y tế, đón trả)
- [ ] 3.8. Dinh dưỡng (thực đơn mẫu → dinh-duong.html)
- [ ] 3.9. Đội ngũ (placeholder, chưa dùng ảnh người thật)
- [ ] 3.10. Gallery preview (3-4 ảnh → gallery.html)
- [ ] 3.11. Testimonial (đánh dấu "nội dung mẫu")
- [ ] 3.12. Học phí teaser + Tuyển sinh (4 bước tóm tắt) + FAQ teaser (3 câu)
- [ ] 3.13. Final CTA: "Đặt lịch tham quan hôm nay" + hotline
...
**Công việc cụ thể:**
- [ ] 4.1. `ve-chung-toi`: câu chuyện, sứ mệnh, giá trị cốt lõi, mục tiêu giáo dục
- [ ] 4.2. `chuong-trinh`: Montessori · STEAM · Giáo dục cảm xúc (EQ) · Vận động · Nghệ thuật · Ngoại ngữ — mỗi chương trình 1 card: mô tả + lợi ích + độ tuổi phù hợp
- [ ] 4.3. `moi-truong`: 8 khu — lớp học, sân chơi ngoài trời, phòng vận động, khu sáng tạo, thư viện, phòng ăn, phòng ngủ, khu vệ sinh
- [ ] 4.4. `hoat-dong`: hoạt động hàng ngày, ngoại khóa, sự kiện
- [ ] 4.5. `dinh-duong`: thực đơn, quy trình bếp, nguyên tắc dinh dưỡng
...
**Công việc cụ thể:**
- [ ] 5.1. `hoc-phi`: bảng học phí công khai (nếu anh cung cấp) HOẶC form nhận bảng giá qua Zalo/Email — **KHÔNG bịa giá**
- [ ] 5.2. `tuyen-sinh`: quy trình 6 bước — Đăng ký → Xác nhận lịch → Tham quan → Tư vấn → Hoàn tất hồ sơ → Nhập học + bộ hồ sơ cần chuẩn bị
- [ ] 5.3. `faq`: 10 câu hỏi accordion (nhập học, bán trú, an toàn, ăn uống, học phí, cam kết…) + FAQPage schema
- [ ] 5.4. `dang-ky-tham-quan`: form — Họ tên PH, SĐT, Email, Tên bé, Ngày sinh, Chương trình quan tâm, Giờ tham quan mong muốn, Ghi chú
...
**Công việc cụ thể:**
- [ ] 6.1. `gallery`: grid/masonry + lightbox + lazy loading + filter theo chủ đề
- [ ] 6.2. `tin-tuc`: danh sách bài viết + phân trang
- [ ] 6.3. 2-3 bài mẫu chuẩn SEO (URL thân thiện, title, description, author, ngày, related posts) — nội dung mẫu rõ ràng
...
**Công việc cụ thể:**
- [ ] 7.1. `form.js`: validation (real-time + submit), error/success/loading states, sanitization
- [ ] 7.2. Conversion hooks: `data-*` attributes cho CTA click, phone click, Zalo click, map click, form submit (sẵn sàng gắn GA4)
- [ ] 7.3. Sticky mobile CTA: [Gọi ngay] [Chat Zalo] [Đăng ký tham quan]
...
**Công việc cụ thể:**
- [ ] 8.1. Meta title/description/H1 duy nhất cho từng trang (đúng cú pháp, đủ độ dài)
- [ ] 8.2. Canonical + Open Graph + Twitter Card
- [ ] 8.3. Schema.org: EducationalOrganization, LocalBusiness, Article, FAQPage, BreadcrumbList
- [ ] 8.4. NAP (Name-Address-Phone) nhất quán toàn site
- [ ] 8.5. Nội dung local thật (tên khu vực, landmarks…) — không spam, không bịa
...
**Công việc cụ thể:**
- [ ] 9.1. Ảnh WebP/AVIF + srcset + lazy loading + preload hero + correct dimensions (chống CLS)
- [ ] 9.2. JS defer, CSS minify, loại bỏ render-blocking
- [ ] 9.3. WCAG: semantic HTML5, keyboard navigation, visible focus, contrast ≥ 4.5:1, aria-label, prefers-reduced-motion
...
- [ ] 10.1. UI: không vỡ layout, không overflow, không ảnh méo/giật, responsive 320→1920px
- [ ] 10.2. UX: mọi link/CTA/menu (desktop+mobile)/form/gallery/lightbox/accordion hoạt động
- [ ] 10.3. SEO: không trùng title, không broken link, sitemap khớp
- [ ] 10.4. Performance: Lighthouse, kiểm tra ảnh nặng
- [ ] 10.5. Accessibility: kiểm tra keyboard/contrast/screen reader cơ bản
- [ ] 10.6. Security: form validation + sanitization, không lộ secret/key, không gửi dữ liệu nhạy cảm kiểu không an toàn
...
**Công việc cụ thể:**
- [ ] 11.1. `README.md`: hướng dẫn chạy local, thay logo, thay ảnh, thay thông tin trường (một chỗ: config), thay màu, thay nội dung, deploy (Netlify/Vercel/GitHub Pages)
- [ ] 11.2. `privacy-policy.html` + `terms.html` + `404.html`
- [ ] 11.3. Checklist nghiệm thu cuối + bàn giao toàn bộ source
